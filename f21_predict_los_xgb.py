...
def save_model(model):
    ...
    keras_filename = output_dir +"/f21_predict_los_xgb.keras"
...
parser.add_argument('--modelfile', type=str, default="output/xgb-los-21cmforest-model.json", help='model file')
...
output_dir = str('output/xgb_los_%s_%s_t%dh_b%d_%s' % (args.runmode, args.telescope,args.t_int, 1, datetime.now().strftime("%Y%m%d%H%M%S")))
if not os.path.exists(output_dir):
    os.mkdir(output_dir)
    print("created " + output_dir)

file_handler = logging.FileHandler(filename=f"{output_dir}/f21_predict_los_xgb.log")
