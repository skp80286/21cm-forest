'''
Predict parameters fX and xHI from the 21cm forest data using Transformer.
'''
...
def load_dataset(datafiles, save=False):
    ...
    processor = dl.F21DataLoader(max_workers=8, psbatchsize=1, limitsamplesize=args.limitsamplesize, skip_ps=True, convert_to_stdev_series=args.convert_to_stdev_series)
...
def save_model(model):
    ...
    keras_filename = output_dir +"/f21_predict_transformer.keras"
...
class TransformerRegressor(Model):
    def __init__(self, seq_len, d_model, num_heads, ff_dim, num_outputs):
        super(TransformerRegressor, self).__init__()
        self.input_projection = layers.Dense(d_model)
        self.attention = layers.MultiHeadAttention(num_heads=num_heads, key_dim=d_model)
        self.ffn = tensorflow.keras.Sequential([layers.Dense(ff_dim, activation='relu'),
                                     layers.Dense(d_model)])
        self.layernorm1 = layers.LayerNormalization(epsilon=1e-6)
        self.layernorm2 = layers.LayerNormalization(epsilon=1e-6)

        self.global_pool = layers.GlobalAveragePooling1D()
        self.output_layer = layers.Dense(num_outputs, activation='linear')
...
def create_transformer_model(input_shape):
    """Create a transformer model for regression"""
    model = TransformerRegressor(seq_len=2762, d_model=256, num_heads=8, ff_dim=512, num_outputs=2)


    # Compile the model  
    model.compile(loss=MeanSquaredError(), optimizer=tensorflow.keras.optimizers.Adam(learning_rate=1e-4), metrics=['accuracy'])
...
parser.add_argument('--modelfile', type=str, default="output/transformer-21cmforest-model.json", help='model file')
...
parser.add_argument('--convert_to_stdev_series', action='store_true', help='convert LoS to stdev series.')

args = parser.parse_args()
output_dir = str('output/transformer_%s_%s_t%dh_b%d_%s' % (args.runmode, args.telescope,args.t_int, 1, datetime.now().strftime("%Y%m%d%H%M%S")))
if not os.path.exists(output_dir):
    os.mkdir(output_dir)
    print("created " + output_dir)

file_handler = logging.FileHandler(filename=f"{output_dir}/f21_predict_transformer.log")
