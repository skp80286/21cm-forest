...
params, freq, los, ps, ks = load_dataset([file_path], psbatchsize=10, limitsamplesize=1000, ps_bins=None)
ks, ps = f21stats.logbin_power_spectrum_by_k(ks, ps)
_, _, losso, psso, _ = load_dataset([so_file_path], psbatchsize=1000, limitsamplesize=1000, ps_bins=None)
_, psso = f21stats.logbin_power_spectrum_by_k(ks, ps)
...
plt.plot(ks[0], psso[0], label=f"Signalonly", color="green", alpha=0.4)
plt.xlabel("k (Hz^-1)")
plt.ylabel("kP(k)")
...
transformed_data = ica.fit_transform(ps)
print(f"Shape of transformed data: {transformed_data.shape}")

sightline_index = 0  # Choose a sightline to compare
noisy_ps = ps[sightline_index]
ks = ks[sightline_index]

# Plot the original and reconstructed sightlines
plt.figure(figsize=(10, 6))
plt.plot(ks, noisy_ps, label="Original PS Sample", color="blue", alpha=0.4)
plt.plot(ks, transformed_data[0]+0.1, label=f"Reconstructed (using mode # {1})", color="red", alpha=0.4)
plt.plot(ks, transformed_data[1]+0.2, label=f"Reconstructed (using mode # {2})", color="blue", alpha=0.4)
plt.plot(ks, transformed_data[2]+0.3, label=f"Reconstructed (using mode # {3})", color="green", alpha=0.4)
plt.plot(ks, transformed_data[3]+0.4, label=f"Reconstructed (using mode # {4})", color="orange", alpha=0.4)
plt.plot(ks, psso[0]+0.5, label=f"Signalonly", color="green", alpha=0.4)
plt.xlabel("k (Hz^-1)")
plt.ylabel("kP(k)")
plt.title(f"Comparison of Original and ICA components\n(z={args.redshift}, xHI_mean={args.xHI}, logfX={args.log_fx})")
plt.legend()
plt.grid(True)
plt.show()
...
for c in range(num_components):
    ...
    component = transformed_data[c].reshape((1,1382))
...
plt.figure(figsize=(10, 6))
plt.plot(ks, noisy_ps, label="Original PS Sample", color="blue", alpha=0.4)
plt.plot(ks, reconstructed_ps[0]+2e-6, label=f"Reconstructed (using mode # {best_component})", color="red", alpha=0.4)
plt.plot(ks, psso[0]+4e-6, label=f"Signalonly", color="green", alpha=0.4)
plt.xlabel("k (Hz^-1)")
plt.ylabel("kP(k)")
