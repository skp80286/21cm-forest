...
for i,datafile in enumerate(datafiles):
    logger.info(f"Loading file: {datafile}")
    ...
    logger.info(f" Saved denoised PS to {denoised_los_file}")
    ...
    denoised_ps_file = f'{denoised_ps_dir}/{file_name_no_ext}.csv' 
    np.savetxt(denoised_ps_file, ps_bin)
    if i == 0:
        np.savetxt(f'{denoised_ps_dir}/ks_bin.csv', ks_bin)
