...
class UnetModel(nn.Module):
    def __init__(self, input_size, input_channels, output_size, dropout=0.2, step=4, latent_dim=256):
        ...
        latent_input_size = self.flat_size//step
        self.fc_enc = nn.Linear(self.flat_size, latent_input_size) 
        self.fc_mu = nn.Linear(latent_input_size, latent_dim)
        self.fc_logvar = nn.Linear(latent_input_size, latent_dim)
        self.fc_pre_dec = nn.Linear(latent_dim, latent_input_size)
        self.fc_dec = nn.Linear(latent_input_size, self.flat_size)
    ...
    def forward(self, x):
        ...
        # Latent space
        flat_enc4 = self.flatten(enc4)
        latent_input = self.fc_enc(flat_enc4)
        mu = self.fc_mu(latent_input)
        logvar = self.fc_logvar(latent_input)
        z = self.reparameterize(mu, logvar)

        # Decode from latent space
        predec = self.fc_pre_dec(z)
        dec1 = self.fc_dec(predec).view(-1, 512, int(self.input_size // (self.step ** 4)))
