...
class ModelTester:
    ...
    def test(self, los, X_test, stats_test, bispectrum_set, y_test, los_so, silent=False):
...
class PCAModel:
    ...
    def fit(self):
        files = glob.glob(f"{args.path}F21_signalonly_21cmFAST_200Mpc_z6.0_fX*_xHI*_8kHz.dat")
        if args.maxfiles is not None:
            files = files[:args.maxfiles]

        logger.info(f"Loading fit data for PCA from {len(files)} files")
        (_, ps, _, _) = load_dataset(files, psbatchsize=1000, limitsamplesize=1000)
        if self.logscale: ps = np.log(np.clip(ps,1e-20,None))

        num_components = min(ps.shape[0], ps.shape[1])
        logger.info(f"PCA with {num_components} components")
        self.model = PCA(n_components=num_components)
        
        tx = self.model.fit(ps)
        #tx = self.model.fit_transform(ps)
        #tx = tx + ps
        #logger.info(f"PCA model fitting: Shape of transformed data: {tx.shape}")
        logger.info(f"PCA model fit done.")
