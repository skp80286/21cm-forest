...
params, freq, los, ps, ks = load_dataset([file_path], psbatchsize=10, limitsamplesize=1000, ps_bins=None, logbinning=args.use_log_binning)
...
component = pca.inverse_transform(transformed_data[:1])#.reshape((1,ps.shape[1]))
print(f"component shape {component.shape}")
mse1 = mean_squared_error(psso, component)
    
subtracted_data = pca.inverse_transform(np.sum(transformed_data[1:], axis=0))
print(f"subtracted shape {subtracted_data.shape}")
mse2 = mean_squared_error(psso, subtracted_data.reshape((1,16)))#.reshape((1,ps.shape[1])))

logger.info(f"orig: {ps_mean}")
logger.info(f"comp1: {component}")
logger.info(f"subtracted: {subtracted_data}")
...
base.initplt()
plt.scatter(ks, noisy_ps, label="Original PS Mean", color="blue", alpha=0.4)
plt.scatter(ks, component, label=f"Reconstructed (using mode # {1})", color="red", alpha=0.4)
plt.scatter(ks, subtracted_data, label=f"Reconstructed by subtraction", color="black", alpha=0.4)
plt.scatter(ks, psso[0], label=f"Signalonly", color="green", alpha=0.4)
plt.loglog(ks, psno[0], "r--", linewidth=0.5, label="Noise")
plt.xlabel("k (Hz^-1)")
plt.ylabel("kP(k)")
plt.title(f"Comparison of Original and  Reconstructed PS\n(z={args.redshift}, xHI_mean={args.xHI}, logfX={args.log_fx})")
plt.legend()
plt.grid(True)
plt.savefig(f"{output_dir}/orig_vs_reconstr.pdf", format = "pdf")
plt.show()
